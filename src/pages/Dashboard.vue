<template>
  <div class="dashboard-container">
    <div class="grid">
      ...
      <div class="col-6 sm:col-4 md:col-3 lg:col-2 flex-auto">
        <div class="nav-tile surface-card shadow-1 hover:shadow-3 border-round p-3 flex flex-column align-items-center justify-content-center transition-colors transition-duration-300 transform-scale-on-hover" @click="router.push({ name: 'Applications' })">
          <div class="mb-2">
            <i class="pi pi-list text-2xl text-primary"></i>
          </div>
          <div class="text-center font-medium">Все заявления</div>
        </div>
      </div>
      
      <div class="col-6 sm:col-4 md:col-3 lg:col-2 flex-auto">
        <div class="nav-tile surface-card shadow-1 hover:shadow-3 border-round p-3 flex flex-column align-items-center justify-content-center transition-colors transition-duration-300 transform-scale-on-hover" @click="router.push({ name: 'Self Applications' })">
          <div class="mb-2">
            <i class="pi pi-user text-2xl text-primary"></i>
          </div>
          <div class="text-center font-medium">Мои заявления</div>
        </div>
      </div>
      
      <div class="col-6 sm:col-4 md:col-3 lg:col-2 flex-auto">
        <div class="nav-tile surface-card shadow-1 hover:shadow-3 border-round p-3 flex flex-column align-items-center justify-content-center bg-green-50 transition-colors transition-duration-300 transform-scale-on-hover" @click="router.push({ name: 'Create application' })">
          <div class="mb-2">
            <i class="pi pi-plus text-2xl text-green-500"></i>
          </div>
          <div class="text-center font-medium">Новое заявление</div>
        </div>
      </div>
      
      <div class="col-6 sm:col-4 md:col-3 lg:col-2 flex-auto">
        <div class="nav-tile surface-card shadow-1 hover:shadow-3 border-round p-3 flex flex-column align-items-center justify-content-center transition-colors transition-duration-300 transform-scale-on-hover" @click="router.push({ name: 'Rups' })">
          <div class="mb-2">
            <i class="pi pi-book text-2xl text-primary"></i>
          </div>
          <div class="text-center font-medium">Учебные планы</div>
        </div>
      </div>
      
      <div class="col-6 sm:col-4 md:col-3 lg:col-2 flex-auto">
        <div class="nav-tile surface-card shadow-1 hover:shadow-3 border-round p-3 flex flex-column align-items-center justify-content-center bg-bluegray-50 transition-colors transition-duration-300 transform-scale-on-hover" @click="router.push({ name: 'Profile' })">
          <div class="mb-2">
            <i class="pi pi-user-edit text-2xl text-bluegray-700"></i>
          </div>
          <div class="text-center font-medium">Профиль</div>
        </div>
      </div>
      <!-- Summary cards -->
          ...
          <template #content>
            ...
            <div class="flex align-items-center">
              <span class="text-green-500 font-medium"
                >+{{ stats.newApplicationsChange }}%</span
              >
      ...
      <div class="col-12 md:col-6 lg:col-3">
        <Card class="mb-0 h-full">
          ...
          <template #content>
            ...
            <div class="flex align-items-center">
              <span class="text-500"
                >Среднее время обработки: {{ stats.avgProcessTime }} дней</span
              >
            </div>
          </template>
        </Card>
        ...
        <Card class="mb-0 h-full">
          ...
          <template #content>
            ...
            <div class="flex align-items-center">
              <span
                :class="
                  stats.rejectionRateChange > 0
                    ? 'text-red-500'
                    : 'text-green-500'
                "
              >
                {{ stats.rejectionRateChange > 0 ? "+" : ""
                }}{{ stats.rejectionRateChange }}%
              </span>
              <span class="text-500 ml-2">от общего числа</span>
            </div>
          </template>
...
const fetchRecentApplications = async () => {
  try {
    ...
    const newAppsChange = totalApps > 0 ? Math.round((applications.filter(app => 
      new Date(app.date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length / totalApps) * 100) : 0;
