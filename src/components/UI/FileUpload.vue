<template>
    <Panel :header="title" class="mb-3">
        ...
        <div v-if="!multiple" class="file-row flex align-items-center p-2 border-0 border-round-sm surface-50 mb-3"
            ...
            <div>
                <Button v-if="props.editable && modelValue && modelValue.length > 0" icon="pi pi-trash" severity="danger" text rounded
                    @click="removeFile(0)" aria-label="Удалить файл" />
                <Button v-else-if='editable' label="Загрузить" icon="pi pi-upload" size="small" outlined
                    @click="triggerFileInput()" />
                <Button v-if='isDownloadBtnShown' @click='onDownloadClick(modelValue[0])' icon='pi pi-download' rounded size='small' severity='secondary'/>
            </div>
        </div>
        ...
        <div v-else>
            <div v-for="(file, fileIndex) in modelValue" :key="fileIndex"
                ...
                <Button v-if='editable' icon="pi pi-trash" severity="danger" text rounded @click="removeFile(fileIndex)"
                    aria-label="Удалить файл" />
                <Button v-if='isDownloadBtnShown' @click='onDownloadClick(file)' icon='pi pi-download' rounded size='small' severity='secondary'/>
            </div>

            <div class="flex justify-content-end mt-3">
                <Button v-if='editable' label="Добавить файл" icon="pi pi-plus" size="small" outlined @click="triggerFileInput()" />
            </div>
        </div>
