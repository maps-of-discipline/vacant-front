    ...
    <Panel>
      <template #header>
        <div class="flex flex-row w-fit align-items-center gap-2">
          ...
          <Help :message="helpText" size='large'/>
        </div>
      </template>

      <div class="flex w-full">
        <Button @click="() => { isDialogShown = true; } " class="m-auto" label="Показать" />
      </div>

      <Dialog class="w-6 min-w-min" v-model:visible="isDialogShown" modal header="Пройденные дисциплины">
        <div class="text-sm">
          <div class="flex align-items-baseline gap-3">
            <span class="text-base">Ед. измерения оъема: </span>
            <SelectButton class="mb-3" v-model="amountMeasure" :allowEmpty="false" :options="['ЗЕТ', 'Ак.Часы']"
              size="small" />
          </div>
          <ScrollPanel style="width: 100%; height: 500px">
            <DataTable size="small" :value="tableData" editMode="cell" @cell-edit-complete="onCellEditComplete" :pt="{
              ...
              <Column v-for="col of columns" :key="col.field" :field="col.field" :header="col.header"
                ...
                <template #body="{ data, field }">
                  <Select v-if="field == 'mark'" v-model="data[field]" :options="markOptions" fluid />
                  <span v-else-if="field == 'amount' && data[field]">{{ data[field] }}
                    {{ amountMeasure == "ЗЕТ" ? " з.е." : " ч." }}</span>
                  <span v-else-if="data[field]">{{ data[field] }}</span>
                  <span v-else class="text-400">-</span>
                </template>
                <template #editor="{ data, field }" v-if="col.field == 'title' || col.field == 'amount'">
                  <InputText v-if="field == 'title'" v-model="data[field]" autofocus fluid placeholder="Название" />
                  <InputNumber v-else-if="field == 'amount'" v-model="data[field]" autofocus fluid />
                </template>
              </Column>
              <Column>
                <template #body="slotProps" class="">
                  <div class="flex justify-content-end gap-2">
                    <Button icon="pi pi-trash" rounded size="small" severity="secondary"
          ...
          <div class="flex p-3 pb-0 jusify-content-center">
            <Button class="m-auto" icon="pi pi-plus" rounded @click="addBlankLine" />
          </div>
        </div>
      </Dialog>
    </Panel>
